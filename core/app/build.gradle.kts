...
android {
  ...
  signingConfigs {
      // CI-safe signing: when real credentials are unavailable (e.g. public
      // GitHub Actions runs without secrets), fall back to dummy passwords so
      // that configuration succeeds. The "custom" config is only *used* when
      // hasCustomSigning is true (see buildTypes below); otherwise the
      // built-in debug keystore signs both debug and release CI artifacts.
      // Production signing behaviour is unchanged when env credentials exist.
      val keyStoreFile = file("${rootProject.projectDir}/signing/signing-key.jks")
      ...
      create("custom") {
          ...
          storePassword = signing_storePassword.ifBlank { "android" }
          keyAlias = "androidcs"
          keyPassword = signing_keyPassword.ifBlank { "android" }
      }
  }
  ...
  // True only when real signing credentials are supplied via environment
  // AND the committed CI keystore exists. Otherwise CI falls back to the
  // auto-generated debug keystore so both APKs still build without secrets.
  val hasCustomSigning =
      (System.getenv("SIGNING_STORE_PASSWORD") ?: "").isNotBlank() &&
      (System.getenv("SIGNING_KEY_PASSWORD") ?: "").isNotBlank() &&
      file("${rootProject.projectDir}/signing/signing-key.jks").exists()

  buildTypes {
    debug {
      signingConfig =
          if (hasCustomSigning) signingConfigs.getByName("custom")
